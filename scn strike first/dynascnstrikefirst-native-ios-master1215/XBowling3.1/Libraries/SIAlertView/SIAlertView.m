...
- (void)validateLayout
{
    ...
    if (self.checkboxBool == YES) {
        if (y > CONTENT_PADDING_TOP) {
            y += GAP;
        }
        UIButton *checkboxButton=[[UIButton alloc]init];
        checkboxButton.backgroundColor=[UIColor redColor];
        checkboxButton.frame=CGRectMake(CONTENT_PADDING_LEFT,y, 30, 27);
        [checkboxButton setImage:[UIImage imageNamed:@"check_off.png"] forState:UIControlStateNormal];
        [checkboxButton setImage:[UIImage imageNamed:@"check_on.png"] forState:UIControlStateSelected];
        [checkboxButton setImageEdgeInsets:UIEdgeInsetsMake(3, 4, 3, 4)];
        [checkboxButton addTarget:self action:@selector(checkboxSelected:) forControlEvents:UIControlEventTouchUpInside];
        [self.containerView addSubview:checkboxButton];
        
        UILabel *label=[[UILabel alloc]init];
        label.frame=CGRectMake(checkboxButton.frame.size.width+checkboxButton.frame.origin.x+30, checkboxButton.frame.origin.y-1, 50, 27);
        label.textColor=[UIColor whiteColor];
        label.text=self.checkboxMessage;
        label.font=self.messageFont;
        label.textColor=self.messageColor;
        label.numberOfLines=MESSAGE_MIN_LINE_COUNT;
        [self.containerView addSubview:label];
        
//        y+=checkboxButton.frame.size.height+checkboxButton.frame.origin.y+GAP;
    }
