...
- (void)viewDidLoad {
    ...
    mainView=[[VIPView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height)];
...
- (void)showLeaderboardForVIP:(NSString *)name
{
    ...
    NSString *siteurl = [NSString stringWithFormat:@"http://xbowling-mobile.trafficmanager.net/VIP/Index?VenueId=15103&Token="];
    ...
    NSString *enquiryurl =   [[siteurl stringByAppendingString:token] stringByAppendingString:apiKey]  ;
    NSLog(@"enquiryurl=%@",enquiryurl);
    
    
    NSURL *url;
    if ([name isEqualToString:@"OC"]) {
        url=[NSURL URLWithString: enquiryurl];//[NSURL URLWithString:@"&apiKey={apiKey}&token={token}"];
